...
- [x] `poe_get_index_status`
  返回索引是否存在、资源数量、索引文件路径、最后更新时间。

- [x] `poe_search_resources`
...
- [x] `poe_datc64_extract_translatable_cells`
...
- [ ] `poe_overlay_prepare_draft`  
  Stage 1 未实现该可选 dry-run 工具；保持未勾选。
  输入候选翻译 JSON，返回拟生成 draft 的摘要、风险、目标资源列表和 diff-like 预览。不得写入 overlay，不得创建文件。

### 明确禁止

- [x] 禁止 `poe_overlay_apply_draft` 出现在 Stage 1。
- [x] 禁止 `poe_run_script`、`poe_write_file`、`poe_generate_tool` 出现在 Stage 1。
- [x] 禁止任何能执行 shell、修改项目文件、修改游戏资源或写 overlay 的工具出现在 Stage 1。
...
- [x] `codex mcp list` 显示 `poe-studio`。
...
- [x] 没有 UI 任务。
- [x] 没有 `/api/agent/*` 或 `/api/codex/*` 任务。
- [x] 没有真实 overlay 写入任务。
- [x] 没有 ToolBuilder、任意脚本执行、任意文件写入工具。
- [x] 每个代码任务都有测试命令和预期结果。
- [x] 每个阶段验收都有可复制命令。
- [x] 计划没有引用当前 clean baseline 不存在的旧 Agent 文件。
